...
void Extruder::manageTemperatures()
{
    ...
    for(uint8_t controller=0; controller<NUM_TEMPERATURE_LOOPS; controller++)
    {
        ...
        if(!Printer::isAnyTempsensorDefect() && (act->currentTemperatureC < MIN_DEFECT_TEMPERATURE || act->currentTemperatureC > MAX_DEFECT_TEMPERATURE))   // no temp sensor tor short in sensor, disable heater
        {
            extruderTempErrors++;
            errorDetected = 1;
...
void TemperatureController::updateCurrentTemperature()
{
    ...
    case 99:
        ...
        //Com::printInfoF(Com::tDebug,osAnalogInputValues[sensorPin]);
        //Com::printInfoFLN(osAnalogInputValues[sensorPin]);
        //currentTemperature /= 1.10;
        break;
    ...
    case 60: // AD8495 (Delivers 5mV/degC vs the AD595's 10mV)
        ...
        //Remove when Thermistor used Ahmet
...
TemperatureController heatedBedController = {NUM_EXTRUDER,HEATED_BED_SENSOR_TYPE,BED_SENSOR_INDEX,-1,-1,0,0,0,0,0,HEATED_BED_HEAT_MANAGER
#ifdef TEMP_PID
